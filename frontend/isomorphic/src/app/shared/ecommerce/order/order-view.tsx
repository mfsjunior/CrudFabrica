...
const orderStatus = [
  { id: 1, label: 'Pendente de aceitação' },
  { id: 2, label: 'Pedido aceito' },
  { id: 3, label: 'Pedido Pago' },
  { id: 3, label: 'Pedido em Andamento' },
  { id: 4, label: 'Pedindo disponibilizado para entrega' },
  { id: 5, label: 'Pedido Entregue' },
];
...
export default function OrderView() {
  ...
  // Procurar o pedido pelo id vindo da URL
  const order = orderData.find((o) => o.id.toString() === orderId);

  if (!order) {
    return <p>Pedido não encontrado.</p>;
  }

  const { products, name } = order;
  ...
  const { price: subtotal } = usePrice({ amount: total });
    ...
    <div className="@container">
      <div className="flex flex-wrap justify-center border-b border-t border-gray-300 py-4 font-medium text-gray-700 @5xl:justify-start">
        ...
        <span className="my-2 ms-5 rounded-3xl border-r border-muted bg-green-lighter px-2.5 py-1 text-xs text-green-dark first:ps-0 last:border-r-0">
          SITUAÇÃO DE PEDIDO: {orderStatus[currentOrderStatus - 1].label}
        </span>
      </div>
          ...
          <div className="pb-5">
            ...
            <div className="border-t border-muted pt-7 @5xl:mt-3">
              <div className="ms-auto max-w-lg space-y-6">
                ...
                <div className="flex justify-between font-medium">
                  Crédito da loja<span>{toCurrency(0)}</span>
                </div>
                <div className="flex justify-between font-medium">
